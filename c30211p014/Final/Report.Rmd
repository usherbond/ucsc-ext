...
The minimum difference between readings of the same day is 5 minutes but there are situations when the reading for the next 5 min is not taken or the period shifts. The following table shows the 5 most common intervals in the data set:

```{r interval_info}
Intervals <- with(rawDataInfo$data,difftime(Time[-1],Time[-length(Time)]))
tmp <- data.frame(head(sort(table(Intervals),decreasing=TRUE)))
knitr::kable(tmp)
