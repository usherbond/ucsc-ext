...
male_covar_rnd = np.matrix( [[ 2.0,  1.80157343],
		 [ 1.80157343,  7.0]] )
...
x = np.matrix([feature_matrix[i] for i in range(len(rnd_gender)) if rnd_gender[i]=='Male'])
#x = np.matrix(feature_matrix)
...
print c
c_alt = (z.transpose() * z)/(z.shape[0]-1)
print c_alt

w_t, v_t = np.linalg.eig(c)
...
#plt.plot(np.array(x[:,0]),np.array(x[:,1]),"bo",alpha=0.1)
#plt.plot(x_mean[0,0],x_mean[0,1],"bs")
#plt.grid()
#plt.show()
...
for i in range(len(w_t)) :
	plt.plot([0,w_t[i]*v_t[0,i]],[0,w_t[i]*v_t[1,i]],'k-',linewidth=2.0)
#plt.plot([0,w_t[0]*v_t[0,0]],[0,w_t[0]*v_t[1,0]],'k-',linewidth=2.0)
#plt.plot([0,w_t[1]*v_t[0,1]],[0,w_t[1]*v_t[1,1]],'k-',linewidth=2.0)
#plt.plot([0,1],[0,1],'k-')
plt.axis('equal')
plt.grid()

f3 = plt.figure(3)
plt.plot(np.array(p[:,0]),np.array(p[:,1]),"bo",alpha=alpha_cloud)
plt.axis('equal')
plt.grid()
...
y_ticks = np.around(bin_center(y,height_bins,min_height,max_height),2)
print "Center height", y_ticks
print np.around(bin_bottom(y,height_bins,min_height,max_height),2)

x = np.asfarray(range(handspan_bins))
x_ticks = np.around(bin_center(x,handspan_bins,min_handspan,max_handspan),2)
print "Center Handspan",x_ticks
print np.around(bin_bottom(x,handspan_bins,min_handspan,max_handspan),2)

print "Python hist"
#py_male_hist,py_male_lim =np.histogramdd(male_df[features].as_matrix(),bins=(height_bins,handspan_bins),range=((min_height,max_height),(min_handspan,max_handspan)))
py_male_hist,py_male_lim =np.histogramdd(male_df[features].as_matrix(),bins=(height_bins,handspan_bins),range=((min_height-0.5*height_width,max_height+0.5*height_width),(min_handspan-0.5*handspan_width,max_handspan+0.5*handspan_width)))
print py_male_hist
#print py_male_lim
if (male_hist==py_male_hist).all() :
	print "Equal"
else :
	print "Different"
	exit(1)
...
#py_female_hist,py_female_lim =np.histogramdd(female_df[features].as_matrix(),bins=(height_bins,handspan_bins),range=((min_height,max_height),(min_handspan,max_handspan)))
py_female_hist,py_female_lim =np.histogramdd(female_df[features].as_matrix(),bins=(height_bins,handspan_bins),range=((min_height-0.5*height_width,max_height+0.5*height_width),(min_handspan-0.5*handspan_width,max_handspan+0.5*handspan_width)))
print py_female_hist
#print py_female_lim
if (female_hist==py_female_hist).all() :
	print "Equal"
else :
	print "Different"
	exit(1)
